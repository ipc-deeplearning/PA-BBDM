...
def _to_3ch_rgb(image):
    """Convert arbitrary-channel tensor (C, H, W) to 3-channel RGB for PIL."""
    if image.shape[0] == 3:
        return image
    if image.shape[0] == 1:
        return image.repeat(3, 1, 1)
    # For >3 channels: take first, middle, last as RGB
    c = image.shape[0]
    idx = [0, c // 2, c - 1]
    return image[idx].contiguous()
...
def get_image_grid(batch, grid_size=4, to_normal=True):
    ...
    # Convert to 3ch before make_grid (which requires 1/3/4 channels)
    batch_3ch = torch.stack([_to_3ch_rgb(b) for b in batch], dim=0)
    image_grid = make_grid(batch_3ch, nrow=grid_size)
    if to_normal:
